...
def create_frame(data, labels, frame_size=256, overlap=128):
    frame = []
    frame_labels = []
...
def create_model(frame_size):
    model = keras.Sequential([
        ...
        # First Conv Block - capture low-level features
...
def train_model(frame_size, X_train, y_train, X_val, y_val, test_participant, scaler):
    ...
    model.compile(
        ...
        loss=keras.losses.BinaryFocalCrossentropy(alpha=0.8, gamma=2.5),
...
def lopo_cross_validation(participant_data, frame_size=256, overlap=128):
    results = {}
...
def main():
    ...
    # model shape checks..
    for filename in os.listdir(trained_model_files):
        if filename.endswith("_model.tflite"):
            participant_id = filename.split("_")[1]
            tflite_path = os.path.join(trained_model_files, filename)
            ...
            print(f"\nModel Details for participant {participant_id}:")
            print("\nInput Details:")
